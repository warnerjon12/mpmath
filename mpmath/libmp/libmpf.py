...
def mpf_floor(s, prec = 0, rnd = round_fast):
    return mpf_rint(s, prec, rnd, round_floor)

def mpf_ceil(s, prec = 0, rnd = round_fast):
    return mpf_rint(s, prec, rnd, round_ceiling)

def mpf_nint(s, prec = 0, rnd = round_fast):
    return mpf_rint(s, prec, rnd, round_nearest)
...
def mpf_eq(s, t):
    ...
    # Quick check: unequal bitcount or NaN
    if s[3] != t[3] or s[3] == -1 or t[3] == -1: return False
    # Both +-0 or the same
    return not s[3] or s == t
